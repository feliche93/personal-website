...
export default function NFTMarketplace() {
  ...
  async function loadNfts() {
    ...
    const items = await Promise.all(data.map(async i => {
      const tokenUri = await tokenContract.tokenURI(i.tokenId) // https://ifp...
  ...
  if (loadingSate === 'loaded' && !nfts.length) return (
            ...
            nfts.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="p-4 bg-black">
                  ...
                  <button className="w-full bg-indigo-500 text-white font-bold py-2 px-12 rounded" onClick={() => buyNft(nft)}>Buy</button>
                </div>
              </div>
            ))
          }
